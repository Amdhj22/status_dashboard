    ...
    console.log(data);
        for (var i = 0; i < data.length; i++) {
            ...
            if (!document.getElementById(data[i] + '_cb')) {
                ...
                label.innerHTML = data[i].slice(data[i].indexOf(':')+1, data[i].indexOf('_info'));
...
socket.on('data', function (filename, data ) {
        if (document.getElementById(filename)) {
            ...
            title.innerHTML = filename.slice(filename.indexOf(':')+1, filename.indexOf('_info'));
